...
namespace WebAPI.Controllers
{
    ...
    public class JobSeekerCvWebSitesController : ControllerBase
    {
        ...
        {
            var result = await _jobSeekerCvWebSiteService.GetListByJobSeekerCvAsync(jobSeekerCvId);
            if (result.Success)
            {
                return Ok(result);
            }
        ...
        [HttpPost("harddelete")]
        public async Task<IActionResult> HardDelete(int jobSeekerCvWebSiteId)
        {
            var result = await _jobSeekerCvWebSiteService.HardDeleteAsync(jobSeekerCvWebSiteId);
            if (result.Success)
            {
                return Ok(result);
            }
